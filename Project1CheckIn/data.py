...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(self.filepath, 'r') as file:
            ...
            self.header = next(read)
            self.header2col = dict()
            self.data = []

            type = next(read)
            data_type = ["Numeric", "Strings"]

            for n in type:
                if n not in data_type:
            ...
            for item in self.headers:
                if item not in self.header2col:
                    self.headers.remove(item)
            
            for line in read:
                str = []
                line = [item.strip() for item in line]
                for cols in self.header2col:
                    str.append(float(line[self.header2col[cols]]))

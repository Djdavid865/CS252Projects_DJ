...
class Analysis:
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        variables = self.data.select_data(data_vars)
        fig, axes = plt.subplots(nrows=length, ncols=length, sharex='col', sharey='row', figsize=fig_sz)
        fig.suptitle(title)
        for i in range(length):
            for j in range(length):
                axes[i, j].scatter(variables[:,j], variables[:,i])
                if i==length-1:
    ...
    def median(self, headers, rows=[]):
        """"Computes the median for each variable in the header"""

...
def count_words(email_path='data/enron'):
    ...
    for Name, subname, fileList in os.walk(email_path):
        for file in fileList: 
            if file == '.DS_Store':
                continue 

            n_files += 1 
            fd = open(os.path.join(Name, file), encoding='latin-1')
...
def find_top_words(word_freq, num_features=200):
    ...
    '''
    top_words = heapq.nlargest(num_features, word_freq, key = word_freq.get)
    counts = []
    for i in range(len(top_words)):
        counts.append(word_freq.get(top_words[i]))
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron'):
    ...
    '''
    word_lenght = len(top_words)

    features = np.zeros([num_emails, word_lenght])
    ...
    for Name, subname, fileList in os.walk(email_path):
        ...
        for file in fileList:
            ...
            for word in words: 
                if word in word_idx:
                    position = word_idx[word]
                    features[idx, position] += 1 
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    idxs_train = features.shape[0]*(1 - test_prop)
    y_ind_train = len(y)*(1 - test_prop)
    inds_train_idx = len(inds)*(1 - test_prop)
    
    idxs_train = int(idxs_train)
    y_ind_train = int(y_ind_train)
    inds_train_idx = int(inds_train_idx)
...
def retrieve_emails(inds, email_path='data/enron'):
    ...
    for folder in folders:
        folder = os.paths.join(paths, folder)
        total_emails += len(os.listdir(folder))

        for filename in os.listdir(folder):

            with open(os.paths.join(folder, filename), 'r') as filedata:
                string = " ".join(filedata.read().split())

                if idx in inds:
                    emails_retrieved.append(string)
                idx += 1

...
David Jin
CS 251 Data Analysis Visualization, Spring 2023
'''
import numpy as np
...
class KNN:
    ...
    def predict(self, data, k):
        ...
        for item in range(data.shape[0]):
            distance = self.exemplars - data[item,:]
            distance = np.square(distance)
            distance = np.sum(distance, axis=1)
            distance = np.sqrt(distance)
            
            sortedInd = np.argsort(distance)
            labels = self.classes[sortedInd[:k]]
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        y_predict = self.predict(X, k).reshape(n_sample_pts, n_sample_pts)

        colorList = ListedColormap(cartocolors.qualitative.Safe_4.mpl_colors)
        fig, ax = plt.subplots()
        pos = ax.pcolormesh(x, y, y_predict , cmap= colorList)
